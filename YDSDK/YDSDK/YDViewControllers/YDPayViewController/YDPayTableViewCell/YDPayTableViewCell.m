...
@implementation YDPayTableViewCell
...
- (void)createCustomCell
{
    ...
    self.circusImageView.image = [YDImage imagesFromCustomBundle:@"yd_circus"];
...
- (void)setcell:(NSDictionary *)dict
{
    ...
    if (dict)
    {
        self.iconImageView.image = [YDImage imagesFromCustomBundle:dict[@"icon"]];
...
- (void)setSelected:(BOOL)selected animated:(BOOL)animated
{
    ...
    if (selected)
    {
        self.selectedImageView.image = [YDImage imagesFromCustomBundle:@"yd_selected"];
    }
    else
    {
