...
@interface YDPasswordViewController ()
<YDTextfieldViewDelegate>
@property(nonatomic , strong)YDTextfieldView * firstPsd;
@property(nonatomic , strong)YDTextfieldView * secondPsd;
...
@implementation YDPasswordViewController
...
- (void)loadView
{
    ...
    self.firstPsd = [[YDTextfieldView alloc]initWithFrame:CGRectMake(0, 120, CGRectGetWidth(self.view.frame), 40)];
    self.firstPsd.delegate = self;
    [self.firstPsd setPlaceholder:@"请输入新密码" imageNamed:@"yd_password"];
    self.firstPsd.textField.secureTextEntry = YES;
    [self.view addSubview:self.firstPsd];
    
    
    self.secondPsd = [[YDTextfieldView alloc]initWithFrame:CGRectMake(0, CGRectGetMaxY(self.firstPsd.frame) + 20, CGRectGetWidth(self.view.frame), 40)];
    self.secondPsd.delegate = self;
    [self.secondPsd setPlaceholder:@"请再次输入新密码" imageNamed:@"yd_password"];
...
- (void)showResultView
{
    ...
    UIImage * image = [YDImage imagesFromCustomBundle:@"yd_confirm"];
...
- (BOOL)checkChangePsd
{
    if ([YDRegular validatePassword:self.firstPsd.textField.text] &&
        [YDRegular validatePassword:self.secondPsd.textField.text] &&
    ...
    {
        [[[YDTipView alloc]initWithMsg:@"请确认两次密码相同"] showAnimation];
...
- (void)changePsdRequest:(YDUserModel *)userModel
{
    ...
    [[YDNetwork network]requestwithParam:param
                                ...
                                response:^(NSDictionary *resObj)
     {
         ...
         dispatch_async(dispatch_get_main_queue(), ^{
             ...
             {
                 [[[YDTipView alloc]initWithMsg:@"密码修改失败"] showAnimation];
             }
...
//    {
//        if (![YDRegular validatePassword:textField.text])
//        {
//            [[[YDTipView alloc]initWithMsg:@"密码格式不正确"] showAnimation];
//        }
//        
//    }
//    
